...
public interface DocumentLookupProvider {
    ...
    /**
     * Returns metadata for all documents with {@code _seq_no > fromSeqNoExclusive}.
     * Default returns empty list (Lucene-only indices handle this via IndexingExecutionEngine).
     */
    default List<DocumentLookupResult> getDocsBySeqNoRange(long fromSeqNoExclusive, IndexReaderProvider.Reader reader, String indexName)
        throws IOException {
        return List.of();
    }
}
